let mapSettings,mapStandings,mapScoreboard,mapTeams,mapDraft,mapTransactions,mapPlayers,extractCallback;_fe5‍.x([["default",()=>_fe5‍.o]]);_fe5‍.w("../helpers/leagueHelper.mjs",[["mapSettings",["mapSettings"],function(v){mapSettings=v}],["mapStandings",["mapStandings"],function(v){mapStandings=v}],["mapScoreboard",["mapScoreboard"],function(v){mapScoreboard=v}],["mapTeams",["mapTeams"],function(v){mapTeams=v}],["mapDraft",["mapDraft"],function(v){mapDraft=v}],["mapTransactions",["mapTransactions"],function(v){mapTransactions=v}]]);_fe5‍.w("../helpers/gameHelper.mjs",[["mapPlayers",["mapPlayers"],function(v){mapPlayers=v}]]);_fe5‍.w("../helpers/argsParser.mjs",[["extractCallback",["extractCallback"],function(v){extractCallback=v}]]);












class LeagueResource {
  constructor(yf) {
    this.yf = yf;
  }

  meta(leagueKey, cb = () => {}) {
    return this.yf
      .api(
        this.yf.GET,
        `https://fantasysports.yahooapis.com/fantasy/v2/league/${leagueKey}/metadata`
      )
      .then((data) => {
        const meta = data.fantasy_content.league[0];

        cb(null, meta);
        return meta;
      })
      .catch((e) => {
        cb(e);
        throw e;
      });
  }

  settings(leagueKey, cb = () => {}) {
    return this.yf
      .api(
        this.yf.GET,
        `https://fantasysports.yahooapis.com/fantasy/v2/league/${leagueKey}/settings`
      )
      .then((data) => {
        const settings = mapSettings(
          data.fantasy_content.league[1].settings[0]
        );
        const league = data.fantasy_content.league[0];

        league.settings = settings;
        cb(null, league);
        return league;
      })
      .catch((e) => {
        cb(e);
        throw e;
      });
  }

  standings(leagueKey, cb = () => {}) {
    return this.yf
      .api(
        this.yf.GET,
        `https://fantasysports.yahooapis.com/fantasy/v2/league/${leagueKey}/standings`
      )
      .then((data) => {
        const standings = mapStandings(
          data.fantasy_content.league[1].standings[0].teams
        );
        const league = data.fantasy_content.league[0];

        league.standings = standings;
        cb(null, league);
        return league;
      })
      .catch((e) => {
        cb(e);
        throw e;
      });
  }

  // h2h only
  scoreboard(leagueKey, ...args) {
    let url = `https://fantasysports.yahooapis.com/fantasy/v2/league/${leagueKey}/scoreboard`;
    const cb = extractCallback(args);

    if (args.length) {
      url += `;week=${args.pop()}`;
    }

    url += "";

    return this.yf
      .api(this.yf.GET, url)
      .then((data) => {
        const week = data.fantasy_content.league[1].scoreboard.week;
        const scoreboard = mapScoreboard(
          data.fantasy_content.league[1].scoreboard[0].matchups
        );
        const league = data.fantasy_content.league[0];

        league.scoreboard = scoreboard;
        league.scoreboard.week = week;
        cb(null, league);
        return league;
      })
      .catch((e) => {
        cb(e);
        throw e;
      });
  }

  teams(leagueKey, cb = () => {}) {
    return this.yf
      .api(
        this.yf.GET,
        `https://fantasysports.yahooapis.com/fantasy/v2/league/${leagueKey}/teams`
      )
      .then((data) => {
        const teams = mapTeams(data.fantasy_content.league[1].teams);
        const league = data.fantasy_content.league[0];
        league.teams = teams;
        cb(null, league);
        return league;
      })
      .catch((e) => {
        cb(e);
        throw e;
      });
  }

  draft_results(leagueKey, cb = () => {}) {
    return this.yf
      .api(
        this.yf.GET,
        `https://fantasysports.yahooapis.com/fantasy/v2/league/${leagueKey}/draftresults`
      )
      .then((data) => {
        const draft = mapDraft(data.fantasy_content.league[1].draft_results);
        const league = data.fantasy_content.league[0];

        league.draft_results = draft;
        cb(null, league);
        return league;
      })
      .catch((e) => {
        cb(e);
        throw e;
      });
  }

  transactions(leagueKey, cb = () => {}) {
    var league_key1 = leagueKey.toString().split(".t.")[0] //JRG
    return this.yf
      .api(
        this.yf.GET,
        `https://fantasysports.yahooapis.com/fantasy/v2/league/${league_key1}/transactions;` //JRG commented

        //`https://fantasysports.yahooapis.com/fantasy/v2/league/${league_key1}/transactions;team_key=${leagueKey};type=waiver`
      )
      .then((data) => {
        const transactions = mapTransactions(
          data.fantasy_content.league[1].transactions
        );
        const league = data.fantasy_content.league[0];

        league.transactions = transactions;
        cb(null, league);
        return league;
      })
      .catch((e) => {
        cb(e);
        throw e;
      });
  }

  transactions2(leagueKey, cb = () => {}) {
    var league_key1 = leagueKey.toString().split(".t.")[0] //JRG
    return this.yf
      .api(
        this.yf.GET,
        `https://fantasysports.yahooapis.com/fantasy/v2/league/${league_key1}/transactions;` //JRG commented
      )
      .then((data) => {
        const transactions = mapTransactions(
          data.fantasy_content.league[1].transactions
        );
        const league = data.fantasy_content.league[0];




        league.transactions = transactions;
        cb(null, league);
        return league;
      })
      .catch((e) => {
        cb(e);
        throw e;
      });
  }

  // WIP... not sure this is useful... certainly doesn't feel good...
  players(leagueKey, ...args) {    
    const cb = extractCallback(args);
    let playerKeys = args.length ? args.shift() : false,
      week = false;

    if (!Array.isArray(playerKeys)) {
      playerKeys = [playerKeys];
    }

    if (args.length) {
      week = args.shift();
    }

    let url = `https://fantasysports.yahooapis.com/fantasy/v2/league/${leagueKey}/players;`;

    url += `player_keys=${playerKeys.join(",")}`;

    url += "/stats";

    if (week) {
      url += `;week=${week}`;
    }

    return this.yf
      .api(this.yf.GET, url)
      .then((data) => {
        // TODO: map players stats here as well
        const players = mapPlayers(data.fantasy_content.league[1].players);

        const league = data.fantasy_content.league[0];

        league.players = players;

        cb(null, league);
        return league;
      })
      .catch((e) => {
        cb(e);
        throw e;
      });
  }

  playersJRG(leagueKey,startVAL) {
    var league_key1 = leagueKey.toString().split(".t.")[0]
    
    let url = `https://fantasysports.yahooapis.com/fantasy/v2/league/${league_key1}/players;status=W;start=${startVAL};sort=PTS`;

    return this.yf
      .api(this.yf.GET, url)
      .then((data) => {
        // TODO: map players stats here as well
        const players = mapPlayers(data.fantasy_content.league[1].players);

        const league = data.fantasy_content.league[0];

        league.players = players;

        //console.log(league);
        return league;
      })
      .catch((e) => {
        throw e;
      });
  }
}

_fe5‍.d(LeagueResource);
